    ...
    if oncreep then
		...
		if self:FasterOnRoad() then
			if (RoadManager ~= nil and RoadManager:IsOnRoad(x, 0, z)) then
				self.groundspeedmultiplier = self.fastmultiplier or 1
			else
				local pos = ground.Map:GetTileAtPoint(x, 0, z)
				if pos == GROUND.ROAD then
					self.groundspeedmultiplier = self.fastmultiplier or 1
				elseif pos == GROUND.SCALE then
					self.groundspeedmultiplier = 2
				else
					self.groundspeedmultiplier = 1
				end
			end
        ...
        --self.groundspeedmultiplier = self:FasterOnRoad() and ((RoadManager ~= nil and RoadManager:IsOnRoad(x, 0, z)) or ground.Map:GetTileAtPoint(x, 0, z) == GROUND.ROAD) and self.fastmultiplier or 1
		--leon mod end
    end
end
