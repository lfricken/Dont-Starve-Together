local function MakeHat(name)
    ...
    local function ruinshat_unproc(inst)
        if inst:HasTag("forcefield") then
            ...
            inst.components.armor:SetAbsorption(TUNING.RuinsHatAbsorption)
            inst.components.armor.ontakedamage = nil

            if inst._task ~= nil then
                inst._task:Cancel()
            end
            inst._task = inst:DoTaskInTime(TUNING.ShieldCooldown, ruinshat_oncooldown)
        end
    end
    ...
    local function ruinshat_proc(inst, owner)
        ...
        inst.components.armor.ontakedamage = function(inst, damage_amount)
            if owner ~= nil and owner.components.sanity ~= nil then
                owner.components.sanity:DoDelta(-damage_amount * TUNING.ShieldSanityDamage, false)
            end
        end
        ...
        inst._task = inst:DoTaskInTime(TUNING.ShieldTime, ruinshat_unproc)
    end

    local function tryproc(inst, owner)
        if inst._task == nil and math.random() < TUNING.ShieldChance then
    ...
    local function ruins()
        ...
        inst:AddComponent("armor")
        inst.components.armor:InitCondition(TUNING.RuinsArmorDurability, TUNING.RuinsHatAbsorption)
