    ...
    local function ruinshat_proc(inst, owner)
        ...
        inst.components.armor.ontakedamage = function(inst, damage_amount)
            if owner then
                ...
                if sanity then
                    local unsaneness = damage_amount * TUNING.ShieldSanityDamage
        ...
        owner:DoTaskInTime(--[[Duration]] TUNING.ShieldTime, function()
            ...
            if inst:IsValid() then
                ...
                owner:DoTaskInTime(--[[Cooldown]] TUNING.ShieldCooldown, function() inst.active = false end)
            end
        end)
    end

    local function tryproc(inst, owner)
        if not inst.active and math.random() < --[[ Chance to proc ]] TUNING.ShieldChance then
